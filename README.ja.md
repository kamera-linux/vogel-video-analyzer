...
- 🎬 **動画注釈（v0.3.0+）** - バウンディングボックスと種ラベル付きの注釈動画を作成
  - タイムスタンプ付き自動出力パス生成（`video.mp4` → `video_annotated_YYYYMMDD_HHMMSS.mp4`）
  - 多言語種ラベルとフラグアイコン（🇬🇧 🇩🇪 🇯🇵）
  - 🏴 **埋め込みフラグレンダリング（v0.5.10+）** - コード内のPNGフラグ、ゼロファイル依存関係
  - 🏴 **ハイブリッドフラグレンダリング（v0.4.2+）** - カスタム--flag-dirへの自動フォールバック付きPNG画像
  - 最適な読みやすさのために設定可能なフォントサイズ
  - 元の動画からのオーディオ保存
  - フリッカーなしの境界ボックスと検出キャッシング
  - 複数の動画のバッチ処理サポート
  - 右寄せの半透明ラベルボックス
- 🌍 **多言語サポート（v0.3.0+）** - 英語、ドイツ語、日本語の鳥の名前と完全なロケール検出
  - 39種類の鳥を完全に翻訳
  - すべての8つのドイツ語モデル鳥に対応（kamera-linux/german-bird-classifier-v2）
  - **日本語（v0.5.10+）**: `ja_JP.utf8` および `ja_JP.eucjp` ロケール自動検出
  - 表示形式: 「EN: Hawfinch / DE: Kernbeißer / 75%」または「🇬🇧 Hawfinch / 🇩🇪 Kernbeißer / 🇯🇵 アオガラ / 72%」
  - `--language ja` または `VOGEL_LANG=ja` 環境変数で言語を設定
...
- **Bandit（コード監査）:** Low 16件, Medium 0件, High 0件
- **pip-audit（依存関係監査）:** 既知の脆弱性なし
- **v0.5.5 の主なセキュリティ修正:**
  - GitHub GraphQL リクエストに明示的なタイムアウトを追加
  - 外部 Chart.js ダウンロード検証を強化（HTTPS + 許可ホスト）
  - `pillow` の最小バージョンを `12.1.1` に更新

報告方法とポリシーは [SECURITY.md](SECURITY.md) を参照してください。
